...
function event(v){if(!obj(v)||!safe(v.eventId)||!E.getItem(v.itemId)||!Number.isFinite(v.at))return null;const i=E.getItem(v.itemId);return {eventId:v.eventId,itemId:i.id,lemmaId:i.lemmaId,sequence:i.sequence,contextClasses:i.trace.map(t=>t.edge),level:E.data.levels.some(x=>x.id===v.level)?v.level:'mixed',mode:v.mode==='transfer'?'transfer':'learn',modality:v.modality==='audio'?'audio':'text',responseMode:v.responseMode==='input'?'input':'choice',response:String(v.response||'').slice(0,200),expected:i.expected,correct:!!v.correct,hinted:!!v.hinted,errorCodes:Array.isArray(v.errorCodes)?v.errorCodes.filter(safe).slice(0,5):[],responseTime:Number.isFinite(v.responseTime)&&v.responseTime>=0?v.responseTime:null,at:v.at,dataVersion:typeof v.dataVersion==='string'?v.dataVersion:E.data.version,transfer:v.mode==='transfer',audioAssetId:null};}
function session(v){
...
function migrate(raw){
 ...
 const map=new Map();for(const v of Array.isArray(raw.events)?raw.events:[]){const e=event(v);if(e&&!map.has(e.eventId))map.set(e.eventId,e);}out.events=[...map.values()].sort((a,b)=>a.at-b.at).slice(-400);
 const ids=new Set(E.data.lemmas.map(l=>l.id));out.exposed=[...new Set([...(Array.isArray(raw.exposed)?raw.exposed:[]),...out.events.map(e=>e.lemmaId)])].filter(x=>ids.has(x));out.updatedAt=stamp(raw.updatedAt);out.session=session(raw.session);
 if(raw.session&&!out.session)out.recovery='Банк заданий обновился или сессия повреждена. История сохранена; начни новый подход.';
 return out;
}
function merge(a,b){const x=migrate(a),y=migrate(b),m=new Map();for(const e of [...x.events,...y.events]){const old=m.get(e.eventId);if(!old||JSON.stringify(e)<JSON.stringify(old))m.set(e.eventId,e);}return {...x,events:[...m.values()].sort((a,b)=>a.at-b.at).slice(-400),exposed:[...new Set([...x.exposed,...y.exposed])],session:x.session||y.session,updatedAt:Math.max(x.updatedAt,y.updatedAt)};}
function putSession(raw,s){const out=migrate(raw),clean=session(s);if(s&&!clean)throw Error('Invalid morph session');out.session=clean;out.updatedAt=Date.now();if(clean)out.exposed=[...new Set([...out.exposed,...clean.queue.map(q=>E.getItem(q.id).lemmaId)])];return out;}
function accept(raw,result){const out=migrate(raw),e=event(result?.event),s=session(result?.session);if(!e||!s||!out.session||out.session.id!==s.id||out.session.cursor!==s.cursor)return {state:out,accepted:false};if(out.events.some(x=>x.eventId===e.eventId)||out.session.phase!=='question')return {state:out,accepted:false};out.events=[...out.events,e].slice(-400);out.session=s;out.updatedAt=e.at;return {state:out,accepted:true,event:e};}
const api={empty,migrate,merge,putSession,accept,session};if(node)module.exports=api;else root.MorphState=api;
