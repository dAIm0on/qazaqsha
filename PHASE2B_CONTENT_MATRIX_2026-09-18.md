...
1. **Текущий lesson-pack + `canonical.js`** — точная допустимая форма и варианты ответа. При конфликте с черновиком/методичкой pack побеждает.
2. **`explain-bank.js` и текущие canonical rule IDs** — границы уже открытого правила и learner-facing объяснение.
3. `Казахский/учебные материалы` + `ЗАДАНИЯ_ВНУТРИ_УРОКОВ_1-1_3-1.md` — программа, словарь, школьный жанр и порядок модулей.
4. `ТЗ_PHRASE_DRILL_и_банки_1-2_3-1_2026-09-17` и карты расширения — закрытые банки, error codes, remediation и дополнительные ограничения.
5. Research/GRAM — только для более ясного объяснения уже открытого правила; никогда не для открытия будущей темы.
...
P1a bank safety: для 1-2 есть 11 уникальных смысловых пар. Поэтому сессия безопасно ограничивается 11 карточками вместо искусственного добора до 12 через зеркальную пару или новый неоткрытый корень. Для 1-3 банк содержит 12 уникальных пар; минимум 70% корней берутся из прошлых уроков. При наличии error profile целевыми помечаются не более 4 слабых пар за сессию.
...
**Guard:** карточки вопроса 2-1 не получают `T10_QUESTION` в `ruleIds`; полный T10 открывается только в 2-3. Узкий вопрос 2-1 проверяется локально в контексте текущего урока.
