...
(function(root){
 ...
 const ROOTS=['кітап','дос','әке','қала','көлік','адам','қыз','жер','оқушы','дәрігер','үй','мектеп'];
 const TAUGHT_ON={
 ...
 function allowedRule(id,catalog){
  ...
  const skill={T2_PLURAL_LDT:'plural',T4_NO_PLURAL_AFTER_NUMBER:'plural_after_num',T6_PERSON_SG:'person_sg',T7_EMES:'emes',
   T20_POSS:'possessive',T21_POSS_ASSIM:'possessive',T23_POSS_PL:'possessive',T12_GLUE:'person_sg'}[id];
 ...
 function forRule(ruleId,opts={}){
  ...
  return Object.keys(table).filter(root=>ROOTS.includes(root)&&root!==taught).map(root=>card(ruleId,root,table[root]));
 }
 function oneForRule(ruleId,opts={}){
  const list=forRule(ruleId,opts);
  return list.length?clone(list[0]):null;
 }
 function oneForPath(chapter,lesson){
  const ids=(chapter&&chapter.rule_ids)||[];
  for(const id of ids){
   const item=oneForRule(id,{});
