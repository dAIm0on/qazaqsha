...
assert.ok(locked.exercise_ids.length >= 6 && locked.exercise_ids.length <= 10);
assert.ok(locked.item_ids.includes('p3-33-d01') && locked.item_ids.includes('p3-33-g01') && locked.item_ids.includes('p3-33-h06') && locked.item_ids.includes('p3-33-i10'));
assert.ok(['p3-33-k1','p3-33-k2','p3-33-k3'].every(id => locked.item_ids.includes(id)));
...
const openAll = H33.build({ events: [done('33-d2', 1), done('33-g2', 2), done('33-h', 3), done('33-i2', 4)] });
...
const sheet = HW.buildPack('3-3', Pack.all(), { sources: { m33: { title: '3-3', url: '#' } } }, { events: [] });
assert.equal(sheet.homework.external_test_url, 'https://batylbol.kz/test/LichPrityazh.html');
assert.ok(sheet.homework.exercise_ids.every(id => Pack.byId(id) || String(id).startsWith('phrase:')));
assert.equal(H33.validate().ok, true);
ok('L33-09 homework stays short and phrases follow the same unlock');
...
assert.ok(/qazaq-offline-live-20260923-lesson33/.test(sw));
