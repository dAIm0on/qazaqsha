...
function question(s){
 ...
 return '<div class="morph-panel"><div class="morph-progress"><span>'+(transfer?'Проверка новых основ':'Практика')+'</span><strong>'+(s.cursor+1)+' / '+s.queue.length+'</strong></div><h2 class="morph-stem" lang="kk">'+esc(item.stem)+'</h2><p>'+esc(item.gloss)+'</p><p class="morph-operation">'+esc(item.operation)+'</p>'+controls+
 (!done&&!transfer?'<button class="text-button" data-morph-hint>Подсказка</button>':'')+(s.hinted&&!done?'<p class="morph-rule">'+esc(E.reason(item))+'</p>':'')+feedback+'<details><summary>Проговорить после ответа</summary><p class="small">Сначала произнеси форму сама. Если на устройстве есть казахский синтез, можно сравнить. Синтез не проверен как эталон и не оценивает твоё произношение.</p>'+(done?'<button class="text-button" data-morph-speak>Озвучить синтезом</button>':'')+'</details></div>';
}
let showHub=true;
function render(){
