...
function question(s){
 ...
 (view.hint?'<button class="text-button" data-morph-hint>Подсказка</button>':'')+(view.reason?'<p class="morph-rule">'+esc(E.reason(item))+'</p>':'')+feedback+(view.speech?'<details><summary>Проговорить после ответа</summary><p class="small">Сначала произнеси форму сама. Если на устройстве есть казахский синтез, можно сравнить. Синтез не проверен как эталон и не оценивает твоё произношение.</p><button class="text-button" data-morph-speak>Озвучить синтезом</button></details>':'')+'</div>';
}
let showHub=true;
function render(){
