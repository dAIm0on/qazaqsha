...
function stage6GuidedComplete(module,resume,plan=P.stage6GuidedPlan(module.id)){
 ...
 return '<div class="morph-panel morph-teach-panel stage5-panel"><button class="text-button" data-teach-close>← К разделу</button><p class="eyebrow">STAGE 6 · С ОПОРОЙ · ГОТОВО</p><h2>Теперь без подсказки</h2><p>Смысловые контрасты: '+e.semanticChecks.correct+' из '+e.semanticChecks.attempts+'. Guided формы: '+e.guided.correct+' из '+e.guided.attempts+'. Исправления после раскрытого ответа не считаются independent evidence.</p><p class="small">Следующий блок использует train-items без подсказки. Repair-леммы исключаются из последующего independent.</p><div class="morph-actions"><button class="secondary-button" data-stage6-restart-guided>Повторить с опорой</button><button class="primary-button" data-stage6-start-choice>Самостоятельно · выбор</button></div></div>';
}
