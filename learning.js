...
function create(api){
 function render(){
  ...
  $('#learn-content').innerHTML=(api.courseJumpMarkup?api.courseJumpMarkup('learn-course-jump'):'')+'<div class="lesson-selector"><label for="micro-lesson">Маленький урок</label><select id="micro-lesson">'+
   [['numbers','Числа'],['vocab','Слова'],['sounds','Сингармонизм'],['plural','Грамматика'],['person','Личные окончания'],['rules','Правила']].map(([id,title])=>'<optgroup label="'+title+'">'+data.lessons.filter(l=>l.topic===id).map(l=>'<option value="'+l.id+'" '+(l.id===lesson.id?'selected':'')+'>'+esc(l.title)+'</option>').join('')+'</optgroup>').join('')+'</select></div>'+
   ...
   '<div class="lesson-actions"><button type="button" class="primary-button" id="start-lesson">Понятно — проверить</button><button type="button" class="secondary-button" id="start-lesson-bank">Практика урока ('+step.questionIds.length+') — другой объём</button><p class="small">Кодирование правила — плитка «Прохождение» на Сегодня. Здесь ответы скрыты. Практика урока не закрывается прохождением и не ставит Good слову.</p></div>'+
