...
function create(api){
 function render(){
  ...
  $('#learn-content').innerHTML=(api.courseJumpMarkup?api.courseJumpMarkup('learn-course-jump'):'')+'<div class="lesson-selector"><label for="micro-lesson">Маленький урок</label><select id="micro-lesson">'+
   ...
   '<div class="lesson-actions"><button type="button" class="secondary-button" id="start-lesson">Практика урока ('+step.questionIds.length+') — другой объём</button><p class="small">Кодирование правила — плитка «Прохождение» на Сегодня. Здесь ответы скрыты. Практика урока не закрывается прохождением и не ставит Good слову.</p></div>'+
