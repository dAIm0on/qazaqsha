...
function create(api){
 function render(){
  ...
  $('#learn-content').innerHTML=(api.courseJumpMarkup?api.courseJumpMarkup('learn-course-jump'):'')+'<div class="lesson-selector"><label for="micro-lesson">Маленький урок</label><select id="micro-lesson">'+
   ...
   '<div class="lesson-actions"><button type="button" class="primary-button" id="start-lesson">Понятно → проверить ('+step.questionIds.length+')</button><p class="small">Можно открыть любой урок, не проходя предыдущие. Чтение само по себе не повышает уровень.</p></div>'+
