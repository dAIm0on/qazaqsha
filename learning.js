...
function create(api){
 function render(){
  ...
  $('#learn-content').innerHTML=(api.courseJumpMarkup?api.courseJumpMarkup('learn-course-jump'):'')+'<div class="lesson-selector"><label for="micro-lesson">Маленький урок</label><select id="micro-lesson">'+
   [['numbers','Числа'],['vocab','Слова'],['sounds','Сингармонизм'],['plural','Грамматика'],['person','Личные окончания']].map(([id,title])=>'<optgroup label="'+title+'">'+data.lessons.filter(l=>l.topic===id).map(l=>'<option value="'+l.id+'" '+(l.id===lesson.id?'selected':'')+'>'+esc(l.title)+'</option>').join('')+'</optgroup>').join('')+'</select></div>'+
