(function(){
 ...
 function install(raws){
   for(const p of prepare(raws)){
     ...
     const qs=p.exercises.map(q=>({...q,id:prefix+q.id,kind:'fields',lessonId:p.lesson_id,source,group:q.id,part:'1',vocabIds:C.words.filter(w=>w.aliases.some(a=>window.TrainerCore.normalize(q.stimulus)===a||q.fields.some(f=>f.answers.some(x=>window.TrainerCore.normalize(x)===a)))).map(w=>w.id),ruleIds:[],phase:'По полученному уроку'}));
