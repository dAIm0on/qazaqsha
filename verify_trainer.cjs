...
assert.ok(/today-collage/.test(dash));
assert.ok(/today-lessons/.test(dash));
assert.ok(/объяснения урока/.test(dash));
assert.ok(/data-action="path"/.test(dash));
assert.ok(/data-action="course:1-1"/.test(dash));
...
assert.ok(/cyrillic|U\+0400-052F/.test(theme));
