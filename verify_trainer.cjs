...
assert.ok(/IBM Plex Serif/.test(theme));
assert.ok(/IBM Plex Sans/.test(theme));
assert.ok(!/Golos Text/.test(theme));
assert.ok(!/Noto Serif/.test(theme));
...
assert.ok(/U\+0400-0405,U\+0407-0455,U\+0457-045F/.test(theme));
...
const pd31Added=PD31.install(openCourse,cat31);
assert.equal(pd31Added.filter(id=>String(id).startsWith('p3-31-g-')).length,18);
assert.equal(openCourse.questions.filter(q=>q.lessonId==='3-1').length,46);
...
assert.ok(/Lesson31Pack\.courseSession/.test(app31Src));
const start31=app31Src.slice(app31Src.indexOf('function startCourse'),app31Src.indexOf('function startTransfer'));
assert.ok(start31.indexOf('restoreLessonPractice(block)')<start31.indexOf('Lesson31Pack.courseSession'));
...
assert.ok(!/assets\/graphics\/g0/.test(fs.readFileSync(path.join(__dirname,'app.js'),'utf8')+fs.readFileSync(path.join(__dirname,'dashboard.js'),'utf8')+fs.readFileSync(path.join(__dirname,'sw.js'),'utf8')));
assert.ok(/\.hero-spot,\.today-option\.hero-spot\{\s*background:var\(--spot,#FF5A1F\)/.test(theme));
assert.ok(!/g02-empty-review\.svg/.test(fs.readFileSync(path.join(__dirname,'dashboard.js'),'utf8')));
assert.ok(/backdrop-filter:blur\(12px\)/.test(theme));
assert.ok(/clamp\(58px,15vw,104px\)/.test(theme));
assert.ok(/\.hero-spot/.test(theme));
assert.ok(/border-bottom:2px solid var\(--spot/.test(theme));
assert.ok(/\.feedback\.error h3\{[\s\S]*background:var\(--spot/.test(theme));
assert.ok(!/#152e56|#06736f|#ffd363/.test(fs.readFileSync(path.join(__dirname,'styles.css'),'utf8')));
assert.ok(!/stylesheet" href="styles.css"/.test(fs.readFileSync(path.join(__dirname,'index.html'),'utf8')));
assert.ok(/ibm-plex-serif-cyrillic-700-normal\.woff2/.test(openSw));
assert.ok(/\.stimulus\[lang="kk"\]\{font-family:"Segoe UI",sans-serif\}/.test(theme));
assert.ok(/\[lang="kk"\]/.test(theme));
assert.ok(!/font-family:Inter/.test(fs.readFileSync(path.join(__dirname,'styles.css'),'utf8')));
assert.ok(!/font-family:"Segoe UI","Noto Serif","Golos Text"/.test(theme));
...
const Bank32=require('./explain-bank.js');
assert.ok(['T24_POSS_BIZ','T25_POSS_SENDER','T26_POSS_OLAR','T27_DEIXIS'].every(id=>Bank32.byId(id)));
assert.equal(Bank32.byHomework('poss_biz').title,Bank32.byId('T24_POSS_BIZ').title);
assert.ok(Bank32.byId('T24_POSS_BIZ').ru_refresh.includes('наш отец'));
assert.ok(Bank32.byId('T24_POSS_BIZ').medium.includes('два места'));
assert.ok(!/рычаг|слот|бирка|алломорф/.test(JSON.stringify(['T24_POSS_BIZ','T25_POSS_SENDER','T26_POSS_OLAR','T27_DEIXIS'].map(id=>Bank32.byId(id)))));
ok('Lesson 3-2 bank T24–T27 present; ru_refresh/medium kept');

const Pack32=require('./lesson32-pack.js');
const Core32=require('./core.js');
const Diag32=require('./diagnostics.js');
assert.ok(Pack32.sessionA().every(q=>/біздің/i.test(q.stimulus+' '+q.explanation)));
assert.ok(Pack32.sessionC().some(q=>/қолдарың/.test(JSON.stringify(q.fields[0].answers))));
assert.ok(!Pack32.sessionC().some(q=>q.fields[0].answers.includes('қолың')));
const ake=Pack32.check('p3-32-a-g6-ake','Біздің әке');
assert.equal(ake.result.correct,false);
assert.equal(ake.errors[0].error_type,'POSS_NO_SUFFIX');
assert.match(Diag32.line('POSS_NO_SUFFIX',ake.question.fields[0].answers[0],'Біздің әке',ake.question),/наклейк/);
const assim=Pack32.check('p3-32-a-g6-kolikimiz','көлікіміз');
assert.equal(assim.errors[0].error_type,'POSS_ASSIM');
const order=Pack32.check('p3-32-b-g6-order','кітабымыздар');
assert.equal(order.errors[0].error_type,'POSS_ORDER');
assert.match(Diag32.line('POSS_ORDER','кітаптарымыз','кітабымыздар',order.question),/кітаптарымыз/);
const kol=Pack32.check('p3-32-c-g6-kolyn','сендердің қолың');
assert.equal(kol.errors[0].error_type,'POSS_2PL_NO_PL');
assert.match(Diag32.line('POSS_2PL_NO_PL','сендердің қолдарың','сендердің қолың',kol.question),/қолдарың/);
const wrong=Pack32.check('p3-32-d-g6-aken','сіздердің әкелерің');
assert.equal(wrong.errors[0].error_type,'POSS_WRONG_PERSON');
const person=Pack32.check('p3-32-d-g6-balanyzdar','сіздің балаңыздар');
assert.equal(person.errors[0].error_type,'PERSON_ON_POSS');
const bare=Pack32.check('p3-32-g-g6-bare','мына — кітап');
assert.equal(bare.errors[0].error_type,'DEIXIS_BARE');
const korshi=Pack32.check('p3-32-x-g6-korshisin','сенің көршісің');
assert.equal(korshi.result.correct,false);
assert.ok(Pack32.check('p3-32-x-g6-korshisin','сенің көршің').result.correct);
assert.ok(Pack32.check('p3-32-a-g2-bala','баламыз').result.correct);
assert.equal(Pack32.check('p3-32-a-g2-bala','балаларымыз').result.correct,false);
assert.ok(Pack32.check('p3-32-c-g2-kol','сендердің қолдарың').result.correct);
ok('Lesson 3-2 stickers hit the piece; дыры баламыз/көршісің not accepted as keys');

const cat32={lessons:['1-1','1-2','1-3','2-1','2-2','2-3','3-1','3-2'].map(id=>({id,active:true}))};
const Gate32=require('./curriculum-gate.js');
assert.ok(Gate32.allows('possessive',cat32));
assert.ok(Gate32.allows('poss_biz',cat32));
assert.ok(Gate32.allows('deixis',cat32));
assert.ok(!Gate32.allows('case',cat32));
assert.ok(!Gate32.allows('labial',cat32));
assert.ok(Gate32.FUTURE.includes('case'));
const PB32=require('./phrase-banks.js');
assert.equal(PB32.forLesson('3-2').length,24);
const PD32=require('./phrase-drill.js');
assert.equal(PD32.forLesson('3-2',cat32).length,24);
assert.ok(PD32.forLesson('3-2',cat32).every(q=>/^phrase:3-2:(kk-ru|ru-kk):\d{2}$/.test(q.id)));
assert.equal(PD32.forLesson('3-2',cat32).filter(q=>q.dir==='kk-ru').length,12);
assert.equal(PD32.forLesson('3-2',cat32).filter(q=>q.dir==='ru-kk').length,12);
assert.ok(PD32.forLesson('3-2').length===0);
const myBook=PD32.forLesson('3-2',cat32).find(q=>q.id==='phrase:3-2:kk-ru:01');
assert.equal(myBook.stimulus,'Біздің әкеміз');
assert.ok(myBook.fields[0].answers.includes('наш отец'));
ok('Lesson 3-2 gate/phrases: 24 directed items, no case, closed without 3-2 catalog');

const UI32=require('./explain-bank-adapter.js');
assert.ok(UI32.OPEN.includes('3-2')&&UI32.COURSE.some(c=>c.id==='3-2'));
const GC32=require('./grammar-chapters.js');
const path32=GC32.LESSONS.find(l=>l.id==='3-2');
assert.equal(path32.chapters.length,10);
assert.deepEqual(path32.chapters.map(c=>c.id),['3-2-a','3-2-b','3-2-c','3-2-d','3-2-e','3-2-f','3-2-g','3-2-h','3-2-i','3-2-j']);
assert.ok(!/рычаг|бирка|алломорф|слот/i.test(JSON.stringify(path32)));
const html32=fs.readFileSync(path.join(__dirname,'index.html'),'utf8');
...
assert.ok(hw32.homework.exercise_ids.includes('p3-32-c-g2-kol'));
assert.ok(AiC.ALLOWED_LESSONS.includes('3-2'));
ok('Lesson 3-2 Learn/Path/Homework wired; no fifth nav view');

assert.equal(cfg.schedule.cleanAnswersToConsolidate,2);
assert.equal(cfg.schedule.learningSessionBlinds,3);
assert.equal(cfg.schedule.learningIntervening,2);
assert.equal(cfg.fsrs.desired_retention,0.90);
const recNew=core.migrateRecord({},1);
assert.ok(core.isDay0Learning(recNew,1));
let learnQ=['probe'];
core.scheduleRepeat(learnQ,0,'probe',1,['f1','f2','f3','f4'],{learning:true,sessionBlinds:1});
assert.ok(learnQ.filter(x=>x==='probe').length>=2);
learnQ=['probe','f1','f2','f3'];
core.scheduleRepeat(learnQ,0,'probe',2,['f1','f2','f3','f4'],{learning:true,sessionBlinds:3});
assert.equal(learnQ.filter(x=>x==='probe').length,1);
const recDue=core.migrateRecord({recall_review_successes:1,last_successful_review:1,correct_streak:2,seen:5,next_review:10},20);
assert.ok(!core.isDay0Learning(recDue,1+2*86400000));
let dueQ=['due'];
core.scheduleRepeat(dueQ,0,'due',1,['x'],{review:true,sessionBlinds:1});
assert.equal(dueQ.filter(x=>x==='due').length,1);
...
const Transfer=require('./transfer-items.js');
const catPause={lessons:['1-1','1-2','1-3','2-1','2-2','2-3','3-1'].map(id=>({id,active:true}))};
const t20=Transfer.forRule('T20_POSS',{catalog:catPause});
assert.ok(t20.length);
assert.ok(t20.some(q=>q.fields[0].answers[0]==='менің досым'));
assert.ok(!t20.some(q=>q.id.endsWith(':әке')));
assert.ok(!t20.some(q=>q.fields[0].answers[0]==='менің кітабым'&&q.transfer_of.taught_on_root==='кітап'));
const t6=Transfer.forRule('T6_PERSON_SG',{catalog:catPause});
assert.ok(t6.some(q=>q.fields[0].answers[0]==='оқушымын'));
assert.ok(!t6.some(q=>q.id.endsWith(':дәрігер')));
const t21=Transfer.forRule('T21_POSS_ASSIM',{catalog:catPause});
assert.ok(t21.some(q=>q.fields[0].answers[0]==='менің көлігім'));
assert.ok(!t21.some(q=>q.fields[0].answers[0]==='менің кітабым'));
const appPause=fs.readFileSync(path.join(__dirname,'app.js'),'utf8');
const dashPause=fs.readFileSync(path.join(__dirname,'dashboard.js'),'utf8');
assert.ok(/Перенос/.test(appPause)&&/startTransfer/.test(appPause));
assert.ok(/'phrase','transfer','contrast'/.test(appPause));
assert.ok(/Готовится к паузе/.test(dashPause)&&/pause-prep/.test(dashPause));
assert.ok(/Скоро пауза/.test(appPause)&&/data-jump-pause/.test(appPause));
assert.ok(!/data-view=["']memory["']/.test(fs.readFileSync(path.join(__dirname,'index.html'),'utf8')));
assert.ok(require('./curriculum-gate.js').INTRODUCED['3-2']);
ok('P0 B: transfer chip uses other roots; Today pause ribbon; 3-2 stays the lesson branch');

const cat32tr={lessons:['1-1','1-2','1-3','2-1','2-2','2-3','3-1','3-2'].map(id=>({id,active:true}))};
const t24=Transfer.forRule('T24_POSS_BIZ',{catalog:cat32tr});
assert.ok(t24.some(q=>q.fields[0].answers[0]==='біздің досымыз'));
assert.ok(t24.some(q=>q.fields[0].answers[0]==='біздің қаламыз'));
assert.ok(t24.some(q=>q.fields[0].answers[0]==='біздің үйіміз'));
assert.equal(t24[0].fields[0].answers[0],'біздің досымыз');
assert.ok(t24.every(q=>q.ruleIds[0]==='T24_POSS_BIZ'));
assert.ok(!t24.some(q=>q.fields[0].answers.some(a=>a==='кітаптар'||a==='баламыз'||a==='көршісің')));
assert.equal(Transfer.forRule('T24_POSS_BIZ',{catalog:catPause}).length,0);
const t25=Transfer.forRule('T25_POSS_SENDER',{catalog:cat32tr});
assert.ok(t25.some(q=>q.fields[0].answers[0]==='сендердің қалаларың'));
assert.ok(t25.some(q=>q.fields[0].answers[0]==='сіздердің аталарыңыз'));
assert.ok(!t25.some(q=>q.id.endsWith(':дос')||q.id.endsWith(':қол')));
const t26=Transfer.forRule('T26_POSS_OLAR',{catalog:cat32tr});
assert.ok(t26.some(q=>q.fields[0].answers[0]==='олардың досы'));
assert.ok(t26.some(q=>q.fields[0].answers[0]==='олардың үйі'));
assert.ok(!t26.some(q=>q.id.endsWith(':іні')||q.id.endsWith(':қала')));
const t27=Transfer.forRule('T27_DEIXIS',{catalog:cat32tr});
assert.ok(t27.some(q=>q.fields[0].answers[0]==='осы қала'));
...
const Repair=require('./repair-state.js');
const Mem=require('./memory-policy.js');
const stRepair=progress.empty();
stRepair.records['probe-t21']={seen:1,recall_review_successes:1,next_review:1};
stRepair.records['word:кітап']={seen:2,next_review:1,fsrs:{scheduled_days:3}};
const probeT21={id:'probe-t21',topic:'plural',kind:'fields',ruleIds:['T21_POSS_ASSIM'],stimulus:'кітабым',fields:[{answers:['менің кітабым']}]};
assert.ok(Mem.mixRulesProbes([],[probeT21],stRepair,{}).includes('probe-t21'));
assert.equal(Repair.start(stRepair,'T21_POSS_ASSIM',Date.now(),['кітап']),true);
assert.equal(Repair.start(stRepair,'T4_NO_PLURAL_AFTER_NUMBER',Date.now(),['дос']),false);
assert.ok(!Mem.mixRulesProbes([],[probeT21],stRepair,{}).includes('probe-t21'));
...
const stSave=progress.empty();
stSave.savings={T2_PLURAL_LDT:2};
assert.equal(Repair.savingsLine(stSave,'T2_PLURAL_LDT'),'T2 savings 2');
assert.ok(Repair.metricParts(stSave,[]).some(part=>part.kind==='savings'&&part.value==='T2 savings 2'));
assert.equal(core.pauseReady({seen:1},Date.now()),false);
assert.equal(core.pauseReady({recall_review_successes:1,last_successful_review:1,fsrs:{scheduled_days:21},next_review:Date.now()},Date.now()),true);
assert.ok(/data-jump-pause/.test(appPause)&&/startPausePrep/.test(appPause));
...
const PathKeep=require('./grammar-path.js');
const parked={lessonId:'3-1',chapterId:'3-1-poss',beat:6,phase:'lesson',completedChapters:{}};
assert.equal(PathKeep.keepChapter(parked,'3-1'),true);
assert.equal(PathKeep.keepChapter(parked,'1-1'),false);
parked.completedChapters['3-1:3-1-poss']=true;
assert.equal(PathKeep.keepChapter(parked,'3-1'),false);
parked.completedChapters={};parked.phase='hub';
assert.equal(PathKeep.keepChapter(parked,'3-1'),false);
const draftState=progress.migrate({schema:6,records:{},grammarPath:{lessonId:'3-1',chapterId:'3-1-poss',beat:6,phase:'beat',pathDraft:{lessonId:'3-1',chapterId:'3-1-poss',beat:6,value:'әкем'},canonShownFor:'3-1:3-1-poss'},place:{surface:'path',lessonId:'3-1',mode:'path'},rulesDraft:{article:'plural',value:'почему дар'}});
assert.equal(draftState.grammarPath.pathDraft.value,'әкем');
assert.equal(draftState.grammarPath.pathDraft.beat,6);
assert.equal(draftState.grammarPath.beat,6);
assert.equal(draftState.grammarPath.canonShownFor,'3-1:3-1-poss');
assert.equal(draftState.place.lessonId,'3-1');
assert.equal(draftState.place.surface,'path');
assert.equal(draftState.rulesDraft.value,'почему дар');
assert.equal(draftState.rulesDraft.article,'plural');
const appStep=fs.readFileSync(path.join(__dirname,'app.js'),'utf8');
const dashStep=fs.readFileSync(path.join(__dirname,'dashboard.js'),'utf8');
const learnStep=fs.readFileSync(path.join(__dirname,'learning.js'),'utf8');
assert.ok(/function continueStep/.test(appStep));
assert.ok(/G\.keepChapter\(gp,lessonId\)/.test(appStep));
assert.ok(/captureDraft\(\);\s*pauseTimer/.test(appStep));
assert.ok(/rulesDraft/.test(appStep));
assert.ok(/dataset\.beat/.test(appStep));
assert.ok(!/G\.startLesson\(state,l\.dataset\.pathLes\)/.test(appStep));
assert.ok(/data-action="continue"/.test(dashStep));
assert.ok(!/lesson\.courseLesson/.test(dashStep));
...
const cat31tracks={lessons:['1-1','1-2','1-3','2-1','2-2','2-3','3-1','3-2'].map(id=>({id,active:true}))};
const PDTracks=require('./phrase-drill.js');
const plain31=PDTracks.session('3-1',{catalog:cat31tracks,count:12,random:()=>0.37,seen_ids:[]});
assert.equal(plain31.filter(q=>q.title==='RU → KK').length,6);
assert.equal(plain31.filter(q=>q.title==='KK → RU').length,6);
const ruAll=PDTracks.forLesson('3-1',cat31tracks).filter(q=>q.title==='RU → KK');
const seenT21=ruAll.filter(q=>(q.ruleIds||[]).includes('T21_POSS_ASSIM')).map(q=>q.id);
const plainSeen=PDTracks.session('3-1',{catalog:cat31tracks,count:12,random:()=>0.37,seen_ids:seenT21});
const weak31=PDTracks.session('3-1',{catalog:cat31tracks,count:12,random:()=>0.37,seen_ids:seenT21,error_profile:['POSS_ASSIM_VOICE']});
assert.equal(plainSeen.filter(q=>q.title==='RU → KK'&&(q.ruleIds||[]).includes('T21_POSS_ASSIM')).length,0);
assert.equal(weak31.filter(q=>q.title==='RU → KK').length,6);
assert.equal(weak31.filter(q=>q.title==='RU → KK'&&(q.ruleIds||[]).includes('T21_POSS_ASSIM')).length,3);
const weak32=PDTracks.session('3-2',{catalog:cat31tracks,count:8,random:()=>0.2,seen_ids:[],error_profile:['POSS_NO_SUFFIX']});
assert.equal(weak32.length,8);
assert.ok((weak32[0].errorTargets||[]).includes('POSS_NO_SUFFIX'));
assert.equal(PDTracks.forLesson('3-2',cat31tracks).length,24);
const appTracks=fs.readFileSync(path.join(__dirname,'app.js'),'utf8');
const learnTracks=fs.readFileSync(path.join(__dirname,'learning.js'),'utf8');
assert.ok(!/topic==='numbers'&&!courseBlock&&mode!=='numbers'/.test(appTracks));
assert.ok(/topic==='numbers'&&!courseBlock&&window\.NumberLadder/.test(appTracks));
assert.ok(/NumberLadder\.allowed\(q,state\)/.test(appTracks));
assert.ok(/data-track/.test(learnTracks));
assert.ok(/courseLesson===lessonId\|\|l\.courseLesson==='bank'/.test(learnTracks));
...
const TutorGap=require('./ai-tutor.js');
TutorGap.reset();
const possQ={id:'poss',ruleIds:['T21_POSS_ASSIM'],fields:[{answers:['кітабым']}],phase3:{error_type:'poss_assim_voice'}};
TutorGap.noteAnswer(possQ,['кітапым'],{correct:false},false,[],1);
TutorGap.noteAnswer(possQ,['кітапым'],{correct:false},false,[],2);
TutorGap.noteAnswer(possQ,['кітапым'],{correct:false},false,[],3);
assert.equal(TutorGap.templateQuestions('POSS_ASSIM_VOICE').some(q=>/үш кітап/.test(JSON.stringify(q))),false);
assert.equal(TutorGap.takeRemediation(new Map()).length,0);
assert.equal(TutorGap.dueRemediation().some(r=>r.error_code==='POSS_ASSIM_VOICE'&&r.remediation_due),true);
const possGap=TutorGap.coverageGaps().find(g=>g.error_code==='POSS_ASSIM_VOICE');
assert.equal(possGap&&possGap.phrase,'Отдельного упражнения для этой ошибки нет.');
assert.equal(possGap.label,'Притяжательное П/К/Қ');
...
const OpenRead=require('./explain-open.js');
const BankRead=require('./explain-bank.js');
const lesson31=OpenRead.fullHtml('T20_POSS');
const lesson32=OpenRead.fullHtml('T24_POSS_BIZ');
assert.ok(lesson31.length>8000);
assert.ok(lesson31.includes('Қазақский этого рода не копирует')||lesson31.includes('рода не копирует'));
assert.ok(lesson31.includes('Семь шагов'));
assert.ok(lesson31.includes('data-canon-source'));
assert.ok(!lesson31.includes('Ниже по-прежнему краткий вид банка'));
assert.ok(!lesson31.includes('Индекс GRAM'));
assert.ok(lesson32.length>15000);
assert.ok(lesson32.includes('Біздің — наш'));
assert.ok(lesson32.includes('Разжёвано для ученика'));
assert.ok(lesson32.includes('Что открыл курс'));
assert.ok(String(BankRead.byId('T20_POSS').medium).length<500);
