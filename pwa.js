(function(){
 ...
 navigator.serviceWorker.register('./sw.js',{updateViaCache:'none'}).then(reg=>{
   ...
   const waiting=()=>{if(reg.waiting){update.hidden=false;update.onclick=()=>{if(!window.dispatchEvent(new Event('qazaq-before-update',{cancelable:true}))){status.textContent='Сначала экспортируй прогресс: сохранить его в браузере не удалось.';return;}reg.waiting.postMessage({type:'ACTIVATE_UPDATE'});};}};waiting();
