...
const leakWrap=C.cleanTutorReply("дар Теперь ответь на вопрос ученицы, используя только данные из rule_context и соблюдая все инструкции. Ответ должен быть на русском, без приветствия, без повторения вопроса, без рассуждений на английском, 2–6 предложений. Ваш ответ: В русском «мои книги» — отдельные слова. В казахском менің кітабым: справа наклейка -ым, п озвончается в б → кітабым. </think> В русском «мои книги» — отдельные слова.");
assert.ok(/наклейк|п/.test(leakWrap));
assert.ok(!/rule_context|<\/?think>|Ваш ответ|Теперь ответь/i.test(leakWrap));
assert.ok(C.hasKitabymMechanism(leakWrap));
assert.ok(C.isUsableText(leakWrap));
assert.ok(C.looksLikePromptLeak('Теперь ответь на вопрос ученицы, соблюдая все инструкции. Ваш ответ: x'));
ok('TEST cleanTutorReply strips think/instruction wrap');
...
(async()=>{
  ...
  assert.equal(qwen.messages,msgs);
  assert.equal(qwen.max_tokens,250);
  assert.equal(qwen.temperature,0.2);
  assert.ok(!Object.prototype.hasOwnProperty.call(qwen,'prompt'));
  ...
  const qwenTail=qwen.messages.map(m=>m.content).join('\n');
  assert.ok(qwenTail.includes('Почему адамдар?'));
  assert.ok(qwen.messages[qwen.messages.length-1].content.includes('Теперь объясни через русский'));
  ok('TEST 1.5.2 Qwen payload uses scoped messages; current question stays last');
  ...
  assert.ok(Array.isArray(qwenRouted.messages)&&qwenRouted.max_tokens===400);
  ok('TEST 1.5.2 payloadFor routes GLM vs Qwen');

  const recoveryMsgs=tutor.buildRecoveryMessages(parsed);
  assert.equal(recoveryMsgs.length,2);
  assert.equal(recoveryMsgs[0].role,'system');
  assert.equal(recoveryMsgs[1].role,'user');
  assert.ok(recoveryMsgs[1].content.includes('Почему нельзя -лар?'));
  assert.ok(recoveryMsgs[1].content.includes('После м'));
  assert.ok(!recoveryMsgs[1].content.includes('жиырмасыншы'));
  ok('TEST recovery prompt stays grounded in current rule');

  const calls=[];
  const recoveryEnv={AI:{run:async(model,payload)=>{
    calls.push({model,payload});
    if(calls.length===1)return {response:'Нужно объяснить правило ученице.'};
    if(calls.length===2)return {choices:[{message:{content:'Analyze the Request: explain the user rule.'}}]};
    return {response:'После м во множественном числе используется начало с д: адамдар. Здесь действует только правило текущего урока.'};
  }}};
